...
class MultiTurtleBotController:
    def __init__(self):
        ...
        for ns in self.robot_namespaces:
            sub = rospy.Subscriber(ns + '/odom', Odometry, self.odom_callback, callback_args=ns)
    ...
    def imu_callback(self, msg, namespace):
        ...
        # rospy.loginfo("%s IMU Linear Acceleration: [%.2f, %.2f, %.2f]", namespace, *linear_acc)
        # rospy.loginfo("%s IMU Angular Velocity: [%.2f, %.2f, %.2f]", namespace, *angular_acc)

    def odom_callback(self, msg, namespace):
        # 获取机器人的位置信息（包括x, y, z坐标）
        position = msg.pose.pose.position
        self.positions[namespace] = (position.x, position.y, position.z)
        
        # 获取机器人的朝向（四元数）
        orientation = msg.pose.pose.orientation
        self.orientations[namespace] = (orientation.x, orientation.y, orientation.z, orientation.w)
