...
def publish_camera_stream():
    ...
    # bridge = CvBridge()
    pub = rospy.Publisher('direct_topic', direct, queue_size=10)
    # 获取当前脚本文件的目录
    ...
    while not rospy.is_shutdown():
        try:
            ...
            # image_msg.header.frame_id = "camera_frame"  # 根据需要设置相机的frame_id

            # 发布图像消息
            # image_pub.publish(image_msg)

            # perspective = cam.to_perspective(frame, (1024, 1280), 0.5)
...
def find_white_circles_using_Hough(img):
    # 应用阈值分割，将图像转为二值图像
    # 假设白色圆的灰度值较高，使用大于200的阈值进行分割
